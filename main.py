...
def send_error_email(error_text, last_product=None):
    ...
    msg = EmailMessage()
    msg["Subject"] = "Going Going Gone Scraper Crashed"
...
def main():
    ...
    #Looks for items that have the same URL but different colors
    for item in urls:
    ...
    #Looks at each of the URLs to get the necessary information
    for base_url, targets in grouped_products.items():
        try:
            ...
            #Searches specificly for the colors we are looking for
            target_colors = {
            ...
            # Gets The Skus with matching colors
            for sku in skus:
                try:
                    attrs = {
                        a["name"]: a["value"]
                    ...
                    #Creates the item to add to the sheet
                    new_item = {
    ...
    #List out all of the skus
    unique_skus = list({item["SKU"] for item in all_items})

    #Chunks the requests to not overload the system
    def chunk(lst, size):
        for i in range(0, len(lst), size):
            yield lst[i:i + size]

    #Part of the request to the webpage
    headers = {
    ...
    #Link the items with their inventory
    ...
    ggg_name = "Quantities "+datetime_string
    final_shoe.to_csv(
        ggg_name,
        index=False
    )
    ...
    spreadsheet = client.open("Going Going Gone Quantities")
    ...
    rr_sheet = create_and_fill_sheet(spreadsheet, ggg_name, final_shoe)
    driver.quit()
    print("CSV saved!")
    send_error_email("Going Going Gone scraper finished successfully at " + datetime_string)
